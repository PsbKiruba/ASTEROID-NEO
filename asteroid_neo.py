...
class SchemaValidationError(RuntimeError):
    """Raised when a generated report or table does not match its expected schema."""


TABLE_SCHEMAS: dict[str, set[str]] = {
    "table_anchor_validation.csv": {
        "cad_date_tdb",
        "cad_jd_tdb",
        "cad_distance_km",
    ...
    "table_surrogate_timeseries.csv": {
        "jd_tdb",
        "calendar_tdb",
        "horizons_distance_au",
        "global_model_distance_au",
        "surrogate_distance_au",
        "conformal_lo_au",
        "conformal_hi_au",
        "residual_km",
        "locality_score",
    },
    "table_publication_orbital_physics.csv": {
        "jd_tdb",
        "calendar_tdb",
    ...
    "table_publication_hypothesis_parameters.csv": {
        "jd_tdb",
        "calendar_tdb",
        "gi_log10_abs",
        "oi_log10_abs",
    ...
    "table_tensorflow_gate_diagnostics.csv": {
        "gate",
        "candidate_blend_strength",
        "candidate_rmse_km",
        "accepted_blend_strength",
        "gate_accepted",
        "reference_rmse_km",
        "best_candidate_rmse_km",
    },
    "table_tensorflow_anchor_synthesis.csv": {
        "cad_date_tdb",
        "cad_distance_km",
        "horizons_sample_date_tdb",
        "sample_time_offset_hours",
        "accepted_surrogate_minus_cad_km",
        "global_gate_accepted",
        "local_gate_accepted",
    },
    "table_covariance_gi_oi_cascade_uncertainty.csv": {
        "cad_date_tdb",
        "cad_distance_km",
        "nominal_two_body_distance_km",
        "two_body_nominal_minus_cad_km",
        "covariance_width90_km",
        "gi_oi_cascade_width90_km",
        "clone_count",
    },
    "table_dynamical_integrator_timeseries.csv": {
        "jd_tdb",
        "calendar_tdb",
        "horizons_geocentric_distance_au",
        "integrated_geocentric_distance_au",
        "integrated_minus_horizons_km",
        "integrated_helio_x_au",
        "integrated_helio_y_au",
        "integrated_helio_z_au",
        "integrated_helio_vx_au_d",
        "integrated_helio_vy_au_d",
        "integrated_helio_vz_au_d",
        "gi_log10_abs",
        "oi_log10_abs",
    },
    "table_dynamical_integrator_anchor_validation.csv": {
        "cad_date_tdb",
        "cad_jd_tdb",
        "cad_distance_km",
        "nearest_sample_offset_hours",
        "integrated_minus_cad_km",
        "horizons_interpolated_minus_cad_km",
    },
}


REPORT_REQUIRED_TOP_LEVEL = {"generated_utc", "target", "standard", "hypothesis", "object"}
REPORT_DYNAMICS_REQUIRED = {
    "enabled",
    "method",
    "n_samples",
    "horizons_step",
...
def validate_csv_table(path: Path) -> None:
    """Validate one generated CSV table against generic and known table schemas."""
    path = Path(path)
    if not path.exists():
        raise SchemaValidationError(f"Missing generated table: {path}")
    with path.open(newline="", encoding="utf-8") as fh:
        reader = csv.DictReader(fh)
        fieldnames = reader.fieldnames or []
        if not fieldnames:
            raise SchemaValidationError(f"{path} has no CSV header")
        if len(fieldnames) != len(set(fieldnames)):
            raise SchemaValidationError(f"{path} has duplicate columns: {fieldnames}")
        required = TABLE_SCHEMAS.get(path.name)
        if required:
            missing = sorted(required.difference(fieldnames))
            if missing:
                raise SchemaValidationError(f"{path} is missing required columns: {', '.join(missing)}")
        row_count = 0
        for row_count, row in enumerate(reader, start=1):
            extra_values = row.get(None)
            if extra_values:
                raise SchemaValidationError(f"{path} row {row_count} has extra CSV fields: {extra_values}")
        if row_count == 0:
            raise SchemaValidationError(f"{path} has a header but no data rows")


def validate_report_dict(report: dict[str, Any]) -> None:
    """Validate the machine-readable report shape used by downstream tools."""
    missing = sorted(REPORT_REQUIRED_TOP_LEVEL.difference(report))
    if missing:
        raise SchemaValidationError(f"report is missing required top-level keys: {', '.join(missing)}")
    dynamics = report.get("dynamics")
    if dynamics is not None:
        if not isinstance(dynamics, dict):
            raise SchemaValidationError("report.dynamics must be an object when present")
        missing_dyn = sorted(REPORT_DYNAMICS_REQUIRED.difference(dynamics))
        if missing_dyn:
            raise SchemaValidationError(f"report.dynamics is missing required keys: {', '.join(missing_dyn)}")
        if int(dynamics.get("n_samples", 0)) <= 1:
            raise SchemaValidationError("report.dynamics.n_samples must be greater than one")
        if not isinstance(dynamics.get("tables"), list):
            raise SchemaValidationError("report.dynamics.tables must be a list")


def validate_report_file(path: Path) -> None:
    path = Path(path)
    if not path.exists():
        raise SchemaValidationError(f"Missing generated JSON report: {path}")
    validate_report_dict(json.loads(path.read_text(encoding="utf-8")))


def validate_output_artifacts(report: AnalysisReport, json_path: Path | None = None) -> None:
    """Validate generated JSON and CSV outputs referenced by a report object."""
    if json_path is not None:
        validate_report_file(json_path)
    validate_report_dict(json.loads(json.dumps(report, default=_json_default)))
    generated_reports: list[Any] = []
    if report.dynamics is not None:
        generated_reports.append(report.dynamics)
    for generated in generated_reports:
        for table in getattr(generated, "tables", []):
            table_path = Path(table)
            if table_path.suffix.lower() == ".csv":
                validate_csv_table(table_path)
...
def _horizons_date(text: str) -> str:
...
def fetch_horizons_vectors(
    ...
    data, url = _http_json(
        HORIZONS_API,
        {
            ...
            "EPHEM_TYPE": "VECTORS",
            "CENTER": center,
            "START_TIME": _horizons_date(date_min),
            "STOP_TIME": _horizons_date(date_max),
            "STEP_SIZE": step,
            "OUT_UNITS": "AU-D",
            "VEC_TABLE": "3",
            "CSV_FORMAT": "YES",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json_path:
        ...
        json_path.write_text(json.dumps(report, default=_json_default, indent=2, sort_keys=True), encoding="utf-8")

    try:
        validate_output_artifacts(report, json_path)
